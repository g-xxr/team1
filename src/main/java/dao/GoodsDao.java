...
/*
 * 항상 dao 페이지 구현할 때 리스트, 페이징, 상세보기, 추가, 수정, 삭제 순서로
 */

public class GoodsDao {
	
	// goodsList 호출 controller
	public ArrayList<HashMap<String,Object>> goodsList(int beginRow, int rowPerPage) throws Exception {
		...
		// 상품과 상품 이미지를 같이 나타내야하기 때문에 두개의 테이블을 조인한다
	...
	// 상품 추가 controller : insertGoods + 상품 이미지까지 함께 (sql 쿼리 2개 각각)
	public void addGoods(String goodsTitle, int goodsPrice, String goodsMemo, String contentType, String filename, String originName) throws Exception{
		...
		// 재고 디폴트 값을 재고있음(N)으로 해두고 나중에 상품별로 수정하기
		...
		// goodsNo는 오토커밋 돼서, soldout은 N이 기본값이어서, createdate, updatedate도 자동으로 입력돼서 작성 제외시킴)
	...
	public void updateGoods(Goods g, String updateName, String name, String contentType, String oldName, String uploadPath) throws Exception {
		...
		stmt1.setInt(5, g.getGoodsNo()); // WHERE 절에 해당하는 값 설정
		...
		if(updateName != null) {
			...
			File old = new File(uploadPath+"/"+oldName);
			if(old.exists()) {
				old.delete();
			}
			...
			stmt2.setInt(4, g.getGoodsNo()); // WHERE 절에 해당하는 값 설정
			int row2 = stmt2.executeUpdate();
			if(row2 != 1) {		// 잘못된 수정 or 실패
				conn.rollback();
				// db 자원 close
				// redirect
				return;
	...
	// getOldFilename 수정 전 저장된 파일 이름 가져오기(oldName)
	...
	// 상품 삭제 controller : deleteGoods (상품, 상품이미지, 장바구니에 담긴 상품 삭제하기)
